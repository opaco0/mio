    ...
    for i, k in enumerate(klines):
        ...
        if should_calc_trades:
            ...
            if trades:
                ...
                # CORREZIONE: Applica filtro a TUTTE le barre
                if filter_mode != "none":
        ...
        bars.append({
            ...
            "volume": round(vol, 2),
            "footprint_volume": round(footprint_volume, 2),
            "levels": levels_data,
            "bullish": c > o,
...
<body>
    ...
    <div class="avg-strength-chart-container" style="margin: 15px 0; padding: 15px; background: #1e1e1e; border-radius: 8px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
            <div style="font-size: 13px; font-weight: 600; color: #fff;">
                Avg Strength Trend
            </div>
            <select id="avgTimeWindow" onchange="renderAvgStrengthChart()" style="background: #2a2a2a; color: #fff; border: 1px solid #444; border-radius: 4px; padding: 3px 8px; font-size: 11px;">
        ...
        <canvas id="avgStrengthChart" width="600" height="200" style="width: 100%; max-height: 200px;"></canvas>
        <div id="avgChartLegend" style="margin-top: 8px; font-size: 11px; color: #aaa; display: flex; gap: 15px; justify-content: center;">
            <span><span style="color: #4caf50; font-size: 14px;">●</span> BUY Avg</span>
            <span><span style="color: #f44336; font-size: 14px;">●</span> SELL Avg</span>
        </div>
    </div>
       ...
       function loadDataLastOnly() {
        ...
        function calculateTradingSignal() {
            ...
            // SEGNALE BUY MODERATO (solo OB forte)
                ...
                signal = 'buy';
                strength = Math.min(60, Math.round((Math.abs(normalizedObDelta) / OB_THRESHOLD) * 50));
            }
        ...
        if (!tracker.currentPhase || tracker.currentPhase.signal !== signal) {
            if (tracker.currentPhase) {
                ...
                if (tracker.phases.length > 50) tracker.phases.shift();
            }
        ...
        // ============================================
        // TRACKING STORICO AVG STRENGTH
        // ============================================

        if (!window.avgStrengthHistory) {
            window.avgStrengthHistory = {
                timestamps: [],
                buyAvg: [],
                sellAvg: [],
                maxHistory: 100
        ...
        const history = window.avgStrengthHistory;
        const now = Date.now();
        ...
        if (signal === 'buy') {
            currentBuyAvg = strength;
        } else if (signal === 'sell') {
            currentSellAvg = strength;
        }

        history.timestamps.push(now);
        history.buyAvg.push(currentBuyAvg);
        history.sellAvg.push(currentSellAvg);

        if (history.timestamps.length > history.maxHistory) {
            history.timestamps.shift();
            history.buyAvg.shift();
            history.sellAvg.shift();
        }
        ...
        function calculateWeightedDistribution() {
            ...
            if (tracker.currentPhase) allPhases.push({...tracker.currentPhase});
            ...
            return {
                weighted: {
                    buy: (buyWeight / totalWeight) * 100,
                    sell: (sellWeight / totalWeight) * 100,
                    neutral: (neutralWeight / totalWeight) * 100
                },
                time: {
                    buy: totalBars > 0 ? (buyBars / totalBars) * 100 : 0,
                    sell: totalBars > 0 ? (sellBars / totalBars) * 100 : 0,
                    neutral: totalBars > 0 ? (neutralBars / totalBars) * 100 : 0
                },
                bars: { buy: buyBars, sell: sellBars, neutral: neutralBars, total: totalBars },
                avgStrength: {
        ...
        function renderPhaseDistribution() {
            ...
            const createSegment = (percent, color, label, tooltip) => {
                if (percent > 0) {
                    ...
                    seg.style.textShadow = '1px 1px 2px rgba(0,0,0,0.5)';
                    seg.style.transition = 'all 0.3s ease';
                    seg.title = tooltip;
                    if (percent > 8) seg.textContent = label;
                    phaseBar.appendChild(seg);
                }
            };

            createSegment(dist.weighted.buy, 'linear-gradient(to right, #26a69a, #4caf50)', 
                         `▲ ${dist.weighted.buy.toFixed(0)}%`,
                         `BUY: ${dist.weighted.buy.toFixed(1)}% (pesato)\n${dist.time.buy.toFixed(1)}% tempo\n${dist.bars.buy} barre\nIntensità media: ${dist.avgStrength.buy.toFixed(0)}%`);

            createSegment(dist.weighted.sell, 'linear-gradient(to right, #ef5350, #f44336)',
                         `▼ ${dist.weighted.sell.toFixed(0)}%`,
                         `SELL: ${dist.weighted.sell.toFixed(1)}% (pesato)\n${dist.time.sell.toFixed(1)}% tempo\n${dist.bars.sell} barre\nIntensità media: ${dist.avgStrength.sell.toFixed(0)}%`);

            createSegment(dist.weighted.neutral, 'linear-gradient(to right, #78909c, #90a4ae)',
                         `⊡ ${dist.weighted.neutral.toFixed(0)}%`,
                         `NEUTRAL: ${dist.weighted.neutral.toFixed(1)}% (pesato)\n${dist.time.neutral.toFixed(1)}% tempo\n${dist.bars.neutral} barre`);
        ...
        // ============================================
        // RENDERING GRAFICO AVG STRENGTH
        // ============================================

        function renderAvgStrengthChart() {
            const canvas = document.getElementById('avgStrengthChart');
            const timeWindowSelect = document.getElementById('avgTimeWindow');

            if (!canvas || !timeWindowSelect || !window.avgStrengthHistory) return;
            ...
            const getX = (t) => pad.left + ((t - minTime) / timeRange) * chartWidth;
            const getY = (v) => pad.top + chartHeight - (v / 100) * chartHeight;
            ...
            const drawLine = (data, color) => {
                ...
                for (let i = 0; i < filtered.timestamps.length; i++) {
                    if (data[i] > 0) {
                        ...
                        if (i === filtered.timestamps.length - 1) {
                            ctx.fillStyle = '#fff';
                            ctx.font = 'bold 11px Arial';
                            ctx.textAlign = 'left';
                            ctx.fillText(data[i].toFixed(0) + '%', x + 8, y + 4);
                            ctx.fillStyle = color;
                        }
                    }
                }
            ...
            drawLine(filtered.sellAvg, '#f44336');
            drawLine(filtered.buyAvg, '#4caf50');
            ...
            if (filtered.timestamps.length > 0) {
                ...
                if (filtered.timestamps.length > 2) {
                    const midTime = new Date((filtered.timestamps[0] + filtered.timestamps[filtered.timestamps.length - 1]) / 2);
    ...
    <!-- Panel Order Block Rilevanti -->
    <div class="order-panel" id="order-panel">
        <div class="order-panel-header">
            <h3>📊 Order Block - Ordini Rilevanti</h3>
            <button onclick="toggleOrderPanel()" class="panel-toggle">▼</button>
        </div>
        <div class="order-panel-content" id="order-panel-content">
            <div class="order-summary" id="order-summary">
                Caricamento...
            </div>
            <div class="order-tables">
                <div class="order-table-container">
                    <h4 class="bid-header">🟢 BIDS (Supporto)</h4>
                    <table class="order-table" id="bids-table">
                        <thead>
                            <tr>
                                <th>Prezzo</th>
                                <th>Quantità</th>
                ...
                <div class="order-table-container">
                    <h4 class="ask-header">🔴 ASKS (Resistenza)</h4>
                    <table class="order-table" id="asks-table">
                        <thead>
                            <tr>
                                <th>Prezzo</th>
                                <th>Quantità</th>
...
def get_relevant_orders():
    """
    Ritorna solo gli ordini più rilevanti dell'orderbook
    attorno al prezzo corrente (±1% del prezzo)
    """
    try:
        # Prendo il prezzo corrente dall'ultima candela
        interval = request.args.get('interval', '1m')
        klines = fetch_klines(interval, limit=1)
        if not klines:
            return jsonify({'error': 'Cannot fetch current price'}), 500

        current_price = float(klines[0][4])  # Close price

        # Fetch orderbook completo
        ob_data = fetch_orderbook()
        if not ob_data or 'bids' not in ob_data or 'asks' not in ob_data:
            return jsonify({'error': 'Cannot fetch orderbook'}), 500

        # Calcola range di prezzo (±1%)
        price_range = current_price * 0.01
        min_price = current_price - price_range
        max_price = current_price + price_range

        # Filtra ordini rilevanti
        relevant_bids = []
        ...
        # Ordina per quantità (più rilevanti prima)
        relevant_bids = sorted(relevant_bids, key=lambda x: x['quantity'], reverse=True)[:30]
        relevant_asks = sorted(relevant_asks, key=lambda x: x['quantity'], reverse=True)[:30]
